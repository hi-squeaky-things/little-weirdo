    ...
    for entry in entries {
        ...
        if let Some(extension) = path.extension() {
            if extension == "json" {
                ...
                let mut buf = [0u8; 256];
                let serialized = postcard::to_slice(&patch, &mut buf).expect("Failed to serialize to postcard");
                
                // Create output filename (same name, different extension)
                let output_path = path.with_extension("lwp");
                
                // Write to file
                let mut postcard_patch_file = File::create(&output_path).expect("Failed to create output file");
                postcard_patch_file.write_all(&serialized).expect("Failed to write to file");
