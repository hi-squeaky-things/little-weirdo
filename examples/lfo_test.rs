...
use little_weirdo::synth::{data::wavetables::{BoxedWavetable, BoxedWavetables}, math::percentage, wavetable_oscillator::{WaveTableLoFreqOscillatorConfig, WaveTableOscillator, WaveTableOscillatorConfig}, Clockable};
use std::{fs, sync::Arc};
...
fn main() {
    ...
    let lfo_config = WaveTableLoFreqOscillatorConfig {
        soundbank_index: 4,
        time: 5, // 0.5 sec 
    };

    let osc_conf = WaveTableOscillatorConfig {
        soundbank_index: 4,
        glide: false,
        glide_rate: 0,
        detune: 0,
