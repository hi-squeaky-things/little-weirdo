...
fn test_patch_deserialization_json() {
    ...
    let patch: Patch =
        serde_json::from_slice(include_bytes!("../examples/soundbank/waveforms/patches/original/bass.json"))
            .unwrap();
...
fn test_patch_deserialization_postcard() {
    ...
    let patch: Patch =
        postcard::from_bytes(include_bytes!("../examples/soundbank/waveforms/patches/src/bass.lwp")).unwrap();
